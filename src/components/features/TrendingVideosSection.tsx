...
import { Play, Eye, Flame, TrendingUp, ChevronRight } from 'lucide-react';
...
export function TrendingVideosSection({ variant = 'compact' }: Props) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: recent } = await supabase.from('posts').select('id, video_url, image_url, views_count, likes_count, content, user_profiles(username, avatar_url)').eq('is_video', true).gte('created_at', since).order('views_count', { ascending: false }).limit(10);
