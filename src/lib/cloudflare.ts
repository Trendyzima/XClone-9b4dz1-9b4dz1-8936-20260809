...
class AuthClient {
  ...
  async getSession() {
    ...
    try {
      ...
      if (!result.user) { saveSession(null); return { data: { session: null }, error: null }; }
  ...
  async signInWithOtp({ email }: { email: string; options?: { shouldCreateUser?: boolean } }) {
    try { await request('/api/auth/send-otp', { method: 'POST', body: JSON.stringify({ email }) }); return { data: {}, error: null }; }
  ...
  async verifyOtp({ email, token }: { email: string; token: string; type?: string }) {
    try {
      ...
      saveSession(result.session); this.emit('SIGNED_IN', result.session);
  ...
  async signInWithPassword({ email, password }: { email: string; password: string }) {
    try {
      const result = await request<{ session: Session; user: CloudflareUser }>('/api/auth/sign-in', { method: 'POST', body: JSON.stringify({ email, password }) });
  ...
  async updateUser({ password, data }: { password?: string; data?: Record<string, any> }) {
    throw new Error('Password/profile updates are exposed as explicit Cloudflare API endpoints and are not implemented by this compatibility method yet.');
  }
...
class CloudflareClient {
  ...
  functions = { invoke: async (name: string, options?: { body?: any }) => {
    try { return { data: await request(`/api/functions/${encodeURIComponent(name)}`, { method: 'POST', body: JSON.stringify(options?.body ?? {}) }), error: null }; }
    catch (error: any) { return { data: null, error }; }
  }};
