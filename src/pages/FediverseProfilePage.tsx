...
function actorUrlOf(actor: any): string {
  return firstHttp(actor?.url, actor?.id, actor?.actor_url, actor?.uri);
}
...
function actorHandle(actor: any, fallbackDomain = ''): string {
  ...
  if (String(acct).includes('@')) return String(acct).replace(/^@/, '');
  const domain = actorDomain(actor, fallbackDomain);
  return domain ? `${acct}@${domain}` : String(acct);
}
...
function mergeActor(value: any, fallback: string): any {
  ...
  const merged = Object.assign({}, ...candidates.reverse(), ...candidates);
  if (!merged.id && !merged.url && fallback) merged.id = fallback;
  return merged;
}
...
export default function FediverseProfilePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const candidates = [canonical, actorTarget].filter(Boolean);
        const rows: any[] = [];
        for (const candidate of candidates) {
          const { data, error } = await supabase
            .from('remote_posts')
            .select('*, remote_accounts(*)')
            .eq('actor_url', candidate)
            .order('published_at', { ascending: false })
            .limit(30);
          if (!error && data) rows.push(...data);
        }
        const unique = [...new Map(rows.map(row => [String(row.object_url ?? row.uri ?? row.url ?? row.id), row])).values()]
          .sort((a, b) => Date.parse(String(b.published_at ?? b.created_at ?? 0)) - Date.parse(String(a.published_at ?? a.created_at ?? 0)));
  ...
  const toggleFollow = async () => {
    if (!ensureAuth() || !canonicalActor || busy) return;
    setBusy(true);
    try {
      if (following) {
        await federation.unfollow(canonicalActor);
        setFollowing(false);
        toast.success('Unfollowed remote profile');
      } else {
        await federation.follow(canonicalActor);
        setFollowing(true);
        toast.success('Follow request sent');
      }
    } catch (e: any) { toast.error(e?.message ?? 'Follow action failed'); }
  ...
  const interact = async (post: any, kind: 'like' | 'repost') => {
    ...
    const next = kind === 'like' ? { ...current, liked: !current.liked } : { ...current, boosted: !current.boosted };
    ...
    <div className="p-4 border-b border-border">
      ...
      {loading ? <div className="py-12 flex justify-center"><Loader2 className="w-7 h-7 animate-spin text-primary" /></div> : <>
        <div className="flex items-start gap-4">
          ...
          <div className="min-w-0 flex-1"><h1 className="text-xl font-bold truncate">{displayName}</h1><p className="text-sm text-muted-foreground truncate">@{actorHandle(actor, domain)}</p><p className="text-xs text-purple-500 flex items-center gap-1 mt-1"><Globe className="w-3 h-3" />{domain || 'remote actor'}</p></div>
          {canonicalActor && <button onClick={toggleFollow} disabled={busy} aria-pressed={following} aria-label={following ? 'Unfollow remote profile' : 'Follow remote profile'} className={`shrink-0 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-full text-sm font-semibold disabled:opacity-60 ${following ? 'border border-border text-muted-foreground bg-background' : 'bg-primary text-primary-foreground'}`}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : following ? <UserMinus className="w-4 h-4" /> : <UserPlus className="w-4 h-4" />}{following ? 'Following' : 'Follow'}</button>}
        </div>
