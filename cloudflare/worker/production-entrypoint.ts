...
function cors(env: Env, methods = 'GET,POST,OPTIONS') { return {'Access-Control-Allow-Origin': env.APP_ORIGIN,'Access-Control-Allow-Methods': methods,'Access-Control-Allow-Headers': 'authorization, apikey, content-type, x-request-id','Access-Control-Expose-Headers': 'x-request-id, retry-after','Access-Control-Max-Age': '86400',Vary: 'Origin'}; }
function requestId(request: Request) { return request.headers.get('x-request-id') || crypto.randomUUID(); }
function json(env: Env, payload: unknown, status = 200, id?: string) { const headers = new Headers({...cors(env),'Content-Type':'application/json; charset=utf-8','Cache-Control':'no-store'}); if(id) headers.set('x-request-id',id); return new Response(JSON.stringify(payload),{status,headers}); }
...
async function health(env: Env, id?: string): Promise<Response> {
  ...
  try { const upstream=await fetch(`${env.SUPABASE_URL}/rest/v1/`,{headers:{apikey:env.SUPABASE_SECRET_KEY,Accept:'application/json','User-Agent':'Testagram-Cloudflare-Worker/1.0'}}); databaseStatus=upstream.status; databaseReachable=upstream.ok; if(!upstream.ok) databaseError=(await upstream.text()).slice(0,300); }
  ...
  return json(env,{ok,service:'testagram-api',gateway:true,federation:true,federationDomain:'federation.testagram.site',gatewayEndpoint:'https://api.testagram.site/api/gateway',database:'supabase',supabaseProjectRef:env.SUPABASE_PROJECT_REF,databaseReachable,databaseStatus,...(databaseError?{databaseError}:{}),jwksReachable,media:'cloudflare-r2',r2Binding,maxMediaBytes:20*1024*1024,edge:'cloudflare'},ok?200:503,id);
}
async function ready(env: Env,id?:string):Promise<Response>{const h=await health(env,id);const body=await h.json() as any;const readyForTraffic=body.ok===true&&body.gateway===true;return json(env,{...body,ready:readyForTraffic},readyForTraffic?200:503,id);}

async function gateway(request: Request,env: Env):Promise<Response>{
  const id=requestId(request); if(request.method==='OPTIONS') return new Response(null,{status:204,headers:{...cors(env),'x-request-id':id}});
  const raw=await request.text(); let input:any; try{input=raw?JSON.parse(raw):{};}catch{return json(env,{ok:false,error:'Invalid JSON gateway request',code:'INVALID_JSON',requestId:id},400,id);}
  ...
  for(let attempt=1;attempt<=attempts;attempt++){
    ...
    try{
      ...
      responseHeaders.set('x-request-id',id); responseHeaders.set('Cache-Control','no-store'); responseHeaders.set('Access-Control-Allow-Origin',env.APP_ORIGIN); responseHeaders.set('Access-Control-Expose-Headers','x-request-id, retry-after');
