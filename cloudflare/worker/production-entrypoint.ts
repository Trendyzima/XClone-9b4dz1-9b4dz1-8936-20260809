...
async function health(env: Env): Promise<Response> {
  ...
  try {
    const upstream = await fetch(`${env.SUPABASE_URL}/rest/v1/`, {
      headers: {
        ...
        'User-Agent': 'Testagram-Cloudflare-Worker/1.0'
      }
    });
  ...
  try {
    const jwks = await fetch(env.SUPABASE_JWKS_URL, {
      headers: { Accept: 'application/json', 'User-Agent': 'Testagram-Cloudflare-Worker/1.0' }
    });
  ...
  return new Response(JSON.stringify({
    ...
    federation: true,
    federationDomain: 'testagram-api.nahashonnyaga794.workers.dev',
    database: 'supabase',
