...
begin
  ...
  select coalesce(sum(amount),0) into v_refunded from public.refund_transactions
   where original_transaction_id=p_original_transaction_id and status='completed';
  ...
  values(v_wallet.id,p_user_id,'refund',p_amount,p_currency,'completed','refund:'||p_idempotency_key,p_original_transaction_id::text,p_reason,jsonb_build_object('original_transaction_id',p_original_transaction_id))
  returning * into v_tx;
