-- Financial adjunct tables required by the hardened Edge Functions.
-- Kept separate from the canonical wallet migration to preserve existing structure.
...
create table if not exists public.ad_revenue_distributions (
  ...
  creator_share numeric(20,8) not null default 0,
  currency text not null default 'USD',
  wallet_transaction_id uuid references public.wallet_transactions(id),
  created_at timestamptz not null default now(),
...
begin
  ...
  if not exists(select 1 from public.wallet_transactions where id=p_original_transaction_id and user_id=p_user_id) then raise exception 'Original transaction not found'; end if;
  ...
  insert into public.wallet_transactions(wallet_id,user_id,type,amount,currency,status,idempotency_key,reference,description)
  values(v_wallet.id,p_user_id,'refund',p_amount,p_currency,'completed','refund:'||p_idempotency_key,p_original_transaction_id::text,p_reason)
  returning * into v_tx;
