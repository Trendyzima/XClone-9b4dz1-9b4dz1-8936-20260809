...
-- Dynamic SQL keeps this migration compatible with deployments where the legacy
-- creator_earnings table already exists with its established shape.

create or replace function public.post_creator_earning_atomic(
  p_user_id uuid,
  p_amount numeric,
  p_source text,
  p_source_id text,
...
declare
  ...
  v_creator_earning uuid;
begin
  if p_amount is null or p_amount <= 0 then raise exception 'Amount must be positive'; end if;

  select wallet_transaction_id into v_existing
    from public.creator_earning_postings
   where source=p_source and source_id=p_source_id and user_id=p_user_id;
  if v_existing is not null then return v_existing; end if;

  if to_regclass('public.creator_earnings') is not null then
    if p_post_id is not null then
      execute 'insert into public.creator_earnings(user_id,source,amount,post_id,status) values ($1,$2,$3,$4,$5) returning id'
        into v_creator_earning using p_user_id,p_source,p_amount,p_post_id,'completed';
    else
      execute 'insert into public.creator_earnings(user_id,source,amount,status) values ($1,$2,$3,$4) returning id'
        into v_creator_earning using p_user_id,p_source,p_amount,'completed';
    end if;
  end if;

  select * into v_wallet from public.user_wallets where user_id=p_user_id for update;
  if not found then
    insert into public.user_wallets(user_id,currency) values(p_user_id,p_currency) returning * into v_wallet;
  end if;

  update public.user_wallets set balance=balance+p_amount,updated_at=now() where id=v_wallet.id;
  insert into public.wallet_transactions(
    wallet_id,user_id,type,amount,currency,status,idempotency_key,metadata
  ) values (
    v_wallet.id,p_user_id,'creator_earning',p_amount,p_currency,'completed',
    'creator:'||p_source||':'||p_source_id,coalesce(p_metadata,'{}'::jsonb)
  ) returning * into v_tx;
...
create or replace function public.post_ad_revenue_atomic(
  p_user_id uuid,
  p_amount numeric,
  p_period_start date,
  p_period_end date,
...
declare
  ...
  v_wallet public.user_wallets;
  v_tx public.wallet_transactions;
  v_source_id text := p_period_start::text||':'||p_period_end::text||':'||p_user_id::text;
begin
  if p_amount is null or p_amount <= 0 then raise exception 'Amount must be positive'; end if;
  select wallet_transaction_id into v_existing from public.ad_revenue_distributions
   where period_start=p_period_start and period_end=p_period_end and user_id=p_user_id;
  if v_existing is not null then return v_existing; end if;
  ...
  values(v_wallet.id,p_user_id,'ad_revenue_share',p_amount,p_currency,'completed','ad:'||v_source_id,jsonb_build_object('period_start',p_period_start,'period_end',p_period_end))
  returning * into v_tx;
