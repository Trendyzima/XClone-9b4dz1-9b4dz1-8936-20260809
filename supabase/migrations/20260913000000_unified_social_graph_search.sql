...
-- Parse #hashtags from every native post so hashtag pages/search never depend on UI code.
...
begin
  ...
  for m in select regexp_matches(source_text,'#([[:alnum:]_]{1,64})','g') loop
    ...
    insert into public.hashtags(tag,last_used_at) values(tag_value,now())
  ...
  where h.id in (select hashtag_id from public.post_hashtags where post_id=new.id)
     or (tg_op<>'INSERT' and h.id in (select hashtag_id from public.post_hashtags ph where ph.post_id=old.id));
  return new;
end;
$$;
...
-- Backfill existing native posts.
do $$
declare
  p record;
  m text[];
...
-- Transactional native follow/unfollow. Protected profiles become pending requests.
...
declare me uuid:=(select auth.uid()); protected boolean:=false; s text; begin
  ...
  if p_follow then
    s:=case when protected then 'pending' else 'accepted' end;
    insert into public.follows(follower_id,following_id,status,accepted_at) values(me,p_following_id,s,case when s='accepted' then now() else null end)
    on conflict(follower_id,following_id) do update set status=excluded.status,accepted_at=excluded.accepted_at;
  else delete from public.follows where follower_id=me and following_id=p_following_id;
  end if;
...
select jsonb_build_object(
  'following',coalesce((select f.status='accepted' from public.follows f where f.follower_id=(select auth.uid()) and f.following_id=p_following_id),false),
  'requested',coalesce((select f.status='pending' from public.follows f where f.follower_id=(select auth.uid()) and f.following_id=p_following_id),false),
  'followed_by',coalesce((select f.status='accepted' from public.follows f where f.follower_id=p_following_id and f.following_id=(select auth.uid())),false),
  'status',coalesce((select f.status from public.follows f where f.follower_id=(select auth.uid()) and f.following_id=p_following_id),'none')
);
$$;

-- Stable unified search envelope. Native and federated records are returned together.
...
with lp as (
 ...
 from public.posts p join public.post_hashtags ph on ph.post_id=p.id join public.hashtags h on h.id=ph.hashtag_id
 where h.tag=lower(regexp_replace(trim(p_tag),'^#',''))
), rp as (
 ...
 from public.federated_objects o where o.object_type='Note' and o.deleted_at is null
 and exists(select 1 from jsonb_array_elements(coalesce(o.tags,'[]'::jsonb)) t where lower(trim(both '#' from coalesce(t->>'name',''))) = lower(regexp_replace(trim(p_tag),'^#','')))
)
select coalesce(jsonb_agg(item order by sort_at desc),'[]'::jsonb) from (select item,sort_at from lp union all select item,sort_at from rp order by sort_at desc limit least(greatest(coalesce(p_limit,40),1),80)) z;
$$;
