...
begin
  ...
  if exists (select 1 from public.profiles where username = candidate and id <> new.id) then
    candidate := left(base_username, 15) || '_' || suffix;
  end if;
...
declare
  ...
  suffix text;
begin
  desired_username := lower(regexp_replace(coalesce(
    nullif(new.raw_user_meta_data ->> 'username', ''),
    nullif(new.raw_user_meta_data ->> 'preferred_username', ''),
    nullif(new.raw_user_meta_data ->> 'user_name', '')
  ), '[^a-z0-9_]+', '_', 'g'));
  ...
  if char_length(desired_username) >= 3 then
    ...
    suffix := substr(replace(new.id::text, '-', ''), 1, 8);
    if exists (select 1 from public.profiles where username = candidate and id <> new.id) then
      candidate := left(candidate, 15) || '_' || suffix;
    end if;
  else
    candidate := null;
  end if;
