...
begin
  ...
  if coalesce(new.activity_type,'') not in ('Create','Update') then
    return new;
  end if;

  object := case
    when jsonb_typeof(activity->'object')='object' then activity->'object'
    else null
  end;
  ...
  insert into public.federated_objects(
    uri,object_type,actor_uri,instance_id,url,content,summary,published_at,updated_at,
    sensitive,tags,raw_object
  ) values (
    object_id,object_type,actor_uri,instance_id,object_url,content,summary,published,updated,
    sensitive,tags,object
  )
...
exception when others then
  -- Never turn a verified ActivityPub receipt into a transport failure.
...
-- Replay any previously received Create/Update activities so the new canonical
-- object cache is populated without requiring federation peers to redeliver.
do $$
declare r record;
begin
  for r in
    select id from public.federation_inbox
    where activity_type in ('Create','Update') and payload is not null
  loop
