...
begin
  ...
  insert into public.wallet_transactions(
    wallet_id,user_id,type,amount,currency,amount_kes,fx_rate,status,
    payment_method,provider,provider_transaction_id,idempotency_key,reference,description,metadata
  ) values(
    v_wallet.id,p_user_id,'deposit',p_amount,coalesce(p_currency,'USD'),p_amount_kes,p_fx_rate,'completed',
    p_provider,p_provider,p_provider_transaction_id,p_idempotency_key,p_reference,p_description,coalesce(p_metadata,'{}'::jsonb)
  ) returning * into v_tx;
