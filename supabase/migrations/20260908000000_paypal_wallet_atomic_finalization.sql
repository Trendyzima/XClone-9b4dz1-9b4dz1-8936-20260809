...
create unique index if not exists paypal_orders_capture_id_key
  on public.paypal_orders(capture_id) where capture_id is not null;
...
-- Keep the two historical PayPal order/capture naming conventions synchronized.
create or replace function public.sync_paypal_order_aliases()
...
create trigger paypal_orders_alias_sync
before insert or update on public.paypal_orders
for each row execute function public.sync_paypal_order_aliases();
...
begin
  ...
  values(p_user_id::text,0,'USD') on conflict(user_id) do nothing;
...
begin
  ...
  select * into v_order from public.paypal_orders
  where coalesce(order_id,paypal_order_id)=p_order_id for update;
  ...
  where id=v_order.wallet_transaction_id for update;
  ...
  update public.wallets
    set balance=v_after,total_deposited=coalesce(total_deposited,0)+v_tx.amount,updated_at=now()
    where id=v_wallet.id;
  update public.wallet_transactions
    set status='completed',balance_before=v_before,balance_after=v_after,provider_status='COMPLETED',payment_method='paypal',completed_at=now()
    where id=v_tx.id;
  update public.paypal_orders
    set status='captured',capture_id=p_capture_id,paypal_capture_id=p_capture_id,captured_at=now(),updated_at=now()
    where id=v_order.id;
