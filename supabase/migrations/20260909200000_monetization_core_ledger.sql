...
-- Canonical provider-agnostic creator economy ledger.
...
create table if not exists public.monetization_ledger (
  ...
  entry_type text not null check (entry_type in ('tip','subscription','super_follow','paid_content','ad_revenue','sponsorship','digital_product','live_event','live_gift','community','affiliate','topup','platform_fee','creator_earning','refund','chargeback','payout','adjustment')),
...
create unique index if not exists monetization_ledger_idempotency_idx on public.monetization_ledger(account_user_id,idempotency_key) where idempotency_key is not null;
create unique index if not exists monetization_ledger_provider_event_idx on public.monetization_ledger(provider,provider_event_id) where provider is not null and provider_event_id is not null;
create index if not exists monetization_ledger_account_idx on public.monetization_ledger(account_user_id,created_at desc);
create index if not exists monetization_ledger_state_idx on public.monetization_ledger(state,available_at);
...
create unique index if not exists monetization_payout_provider_idx on public.monetization_payouts(provider,provider_payout_id) where provider is not null and provider_payout_id is not null;
create unique index if not exists monetization_payout_idempotency_idx on public.monetization_payouts(user_id,idempotency_key) where idempotency_key is not null;
...
create policy monetization_accounts_read_own on public.monetization_accounts for select to authenticated using ((select auth.uid()) = user_id);
create policy monetization_ledger_read_own on public.monetization_ledger for select to authenticated using ((select auth.uid()) = account_user_id or (select auth.uid()) = counterparty_user_id);
create policy monetization_payouts_read_own on public.monetization_payouts for select to authenticated using ((select auth.uid()) = user_id);

-- Client-safe account bootstrap. Financial writes remain server-only.
create or replace function public.ensure_monetization_account(p_user_id uuid)
returns public.monetization_accounts language plpgsql security definer set search_path=''
as $$
...
begin
  ...
  insert into public.monetization_accounts(user_id) values(p_user_id) on conflict(user_id) do nothing;
...
-- Server-only earning ingestion. This prevents an authenticated client from crediting arbitrary creators.
create or replace function public.record_creator_earning(p_creator_id uuid,p_entry_type text,p_gross_cents bigint,p_idempotency_key text,p_provider text default null,p_provider_event_id text default null,p_source_type text default null,p_source_id text default null,p_description text default '',p_metadata jsonb default '{}'::jsonb,p_creator_share_bps integer default null)
returns public.monetization_ledger language plpgsql security definer set search_path=''
as $$
declare s public.monetization_settings; e public.monetization_ledger; share_bps integer; creator_cents bigint; fee_cents bigint;
begin
...
create or replace function public.request_monetization_payout(p_amount_cents bigint,p_provider text,p_destination jsonb default '{}'::jsonb,p_idempotency_key text default null)
returns public.monetization_payouts language plpgsql security definer set search_path=''
as $$
declare a public.monetization_accounts; s public.monetization_settings; p public.monetization_payouts;
begin
  if (select auth.uid()) is null then raise exception 'unauthorized'; end if;
  if p_amount_cents <= 0 then raise exception 'invalid payout amount'; end if;
  if p_idempotency_key is not null then select * into p from public.monetization_payouts where user_id=(select auth.uid()) and idempotency_key=p_idempotency_key; if found then return p; end if; end if;
  select * into s from public.monetization_settings where id=true;
  select * into a from public.monetization_accounts where user_id=(select auth.uid()) for update;
  if not found then raise exception 'monetization account not found'; end if;
  if p_amount_cents < s.minimum_payout_cents then raise exception 'minimum payout is % cents',s.minimum_payout_cents; end if;
  if p_amount_cents > a.available_cents then raise exception 'insufficient available balance'; end if;
  insert into public.monetization_payouts(user_id,amount_cents,currency,provider,destination,idempotency_key) values((select auth.uid()),p_amount_cents,a.currency,p_provider,coalesce(p_destination,'{}'::jsonb),p_idempotency_key) returning * into p;
  insert into public.monetization_ledger(account_user_id,entry_type,direction,amount_cents,currency,state,source_type,source_id,description,metadata,settled_at) values((select auth.uid()),'payout','debit',p_amount_cents,a.currency,'settled','payout',p.id::text,'Creator payout reservation',jsonb_build_object('provider',p_provider),now());
  update public.monetization_accounts set available_cents=available_cents-p_amount_cents,lifetime_paid_cents=lifetime_paid_cents+p_amount_cents,updated_at=now() where user_id=(select auth.uid());
  return p;
end; $$;
...
begin
  if (select current_user) <> 'service_role' then raise exception 'server_only'; end if;
  for e in select * from public.monetization_ledger where state='pending' and available_at is not null and available_at<=now() order by available_at asc limit greatest(least(p_limit,5000),1) for update skip locked loop
    update public.monetization_ledger set state='available',settled_at=now() where id=e.id;
    update public.monetization_accounts set pending_cents=greatest(pending_cents-e.amount_cents,0),available_cents=available_cents+e.amount_cents,updated_at=now() where user_id=e.account_user_id;
    released:=released+1;
  end loop;
  return released;
end; $$;
...
create or replace view public.creator_monetization_summary with (security_invoker=true) as
select a.user_id,a.currency,a.available_cents,a.pending_cents,a.lifetime_earned_cents,a.lifetime_paid_cents,
...
from public.monetization_accounts a left join public.monetization_ledger l on l.account_user_id=a.user_id
