...
Deno.serve(async req => {
  ...
  try {
    ...
    const amountCents = Math.round(amount * 100); if (amountCents <= 0) return json({ error: "Invalid amount" }, 400);
    ...
    const orderPayload = { intent: "CAPTURE", purchase_units: [{ reference_id: wallet.id, custom_id: user.id, amount: { currency_code: currency, value: amount.toFixed(2) }, description: "Testagram wallet top-up" }], application_context: experience };
