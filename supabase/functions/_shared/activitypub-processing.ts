...
function pemToBytes(pem: string) {
  const b = pem.replace(/-----BEGIN PUBLIC KEY-----|-----END PUBLIC KEY-----|\s/g, '');
...
async function localActorForObject(admin: any, objectUri: string) {
  const { data, error } = await admin
    ...
    .select('*')
    .or(`uri.eq.${encodeURIComponent(objectUri)},actor_url.eq.${encodeURIComponent(objectUri)}`)
    .limit(1)
    .maybeSingle();
...
async function remoteActor(admin: any, actorUri: string) {
  ...
  const { error } = await admin
    ...
    .upsert({ domain, status: 'active', last_seen_at: now(), updated_at: now() }, { onConflict: 'domain' });
  if (error) throw error;
  return a;
}
...
async function signAndSend(local: any, inbox: string, activity: any) {
  ...
  return federationFetch(inbox, {
    ...
    headers: {
      ...
      'User-Agent': 'Testagram-Federation/5.0',
    },
  });
}
...
async function recordDelivery(admin: any, local: any, activity: any, inbox: string, response: Response | null, error?: string) {
  const { data, error: insertError } = await admin
    ...
    .insert({
      ...
      actor_uri: local.uri || local.actor_url,
      object_uri: uri(activity.object) || null,
      target_uri: uri(activity.target) || null,
  ...
  if (!data?.id) return;
  await admin.from('federation_deliveries').upsert({
    activity_id: data.id,
    target_inbox: inbox,
    instance_domain: new URL(inbox).hostname,
    status: response?.ok ? 'delivered' : 'retry',
    attempt_count: 1,
...
async function persistInboundObject(admin: any, actorUri: string, object: any, instanceId: string | null) {
  ...
  const allowed = ['Note', 'Article', 'Question', 'Video', 'Image'];
  ...
  const { error } = await admin.from('federated_objects').upsert({
    ...
    url: text(object?.url) || id,
    content: text(object?.content) || text(object?.name) || null,
    summary: text(object?.summary) || null,
    published_at: object?.published ? new Date(object.published).toISOString() : null,
    updated_at: object?.updated ? new Date(object.updated).toISOString() : now(),
...
async function materializeReplyAncestors(admin: any, actorUri: string, parentUri: string, instanceId: string | null, depth: number): Promise<void> {
  ...
  if (error) throw error;
  if (existing?.uri) {
    if (existing.in_reply_to_uri) await materializeReplyAncestors(admin, actorUri, existing.in_reply_to_uri, instanceId, depth + 1);
    return;
  }
...
export async function processActivity(admin: any, a: any, actorUri: string) {
  ...
  const o = a.object;
  const objectUri = uri(o);
  const targetUri = uri(a.target);
  ...
  if (type === 'Create' || type === 'Update') {
    ...
    if (saved) {
      const parent = uri(obj.inReplyTo);
      if (parent) await materializeReplyAncestors(admin, actorUri, parent, actorRow.instance_id, 1);
    }
    return;
  }
  ...
  if (type === 'Like' || type === 'Announce') {
    ...
    const { error } = await admin.from('federated_interactions').upsert({ activity_uri: a.id, activity_type: type, actor_uri: actorUri, object_uri: objectUri, active: true, raw_activity: a, updated_at: now() }, { onConflict: 'activity_uri' });
    if (error) throw error;
    return;
  }
  ...
  if (type === 'Undo') {
    ...
    if (original.activity_type === 'Follow') {
      const target = uri((original.raw_activity as any).object);
      const local = await localActorForObject(admin, target);
      ...
      throw new Error('Undo type is unsupported');
    }
    return;
  }
