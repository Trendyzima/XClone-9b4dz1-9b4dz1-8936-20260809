...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const fxRate = await getFxRate(supabaseAdmin);
    ...
    if (existing) {
      return new Response(JSON.stringify({
        ...
        conversation_id: existing.provider_transaction_id ?? existing.reference ?? null,
    ...
    await supabaseAdmin.from('platform_inbox').insert({
      ...
      body: `Your withdrawal of KES ${intAmount.toLocaleString()} ($${usdAmount.toFixed(2)}) to M-Pesa number ${normalisedPhone} has been initiated.`,
      type: 'system',
      icon_emoji: '💸',
    });
