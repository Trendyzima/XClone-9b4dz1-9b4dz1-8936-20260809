...
async function getFxRate(supabaseAdmin: ReturnType<typeof createClient>): Promise<number> {
  const {data}=await supabaseAdmin.from('platform_exchange_rates').select('rate').eq('base_currency','USD').eq('quote_currency','KES').lte('effective_at',new Date().toISOString()).order('effective_at',{ascending:false}).limit(1).maybeSingle();
  return Number(data?.rate)>0?Number(data.rate):DEFAULT_FX_RATE;
}

Deno.serve(async(req:Request)=>{
  if(req.method==='OPTIONS')return new Response('ok',{headers:corsHeaders});
  const supabaseAdmin=createClient(Deno.env.get('SUPABASE_URL')??'',Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')??'');
  let reservedTransactionId:string|null=null; let mpesaAccepted=false;
  try{
    ...
    await enforceRateLimit(supabaseAdmin,`mpesa:b2c:user:${user.id}`,5,300);

    const body=await req.json(); const {phone,amount,purpose,idempotency_key}=body;
    ...
    const fxRate=await getFxRate(supabaseAdmin); const usdAmount=intAmount/fxRate;
    const idempotencyKey=String(idempotency_key??req.headers.get('Idempotency-Key')??crypto.randomUUID());

    const {data:existing}=await supabaseAdmin.from('wallet_transactions').select('id,status,provider_transaction_id,reference').eq('user_id',user.id).eq('idempotency_key',idempotencyKey).maybeSingle();
    ...
    const {data:reservedId,error:reserveErr}=await supabaseAdmin.rpc('reserve_wallet_funds',{p_user_id:user.id,p_amount:usdAmount,p_idempotency_key:idempotencyKey});
    ...
    const b2cPayload={InitiatorName:initiatorName,SecurityCredential:securityCred,CommandID:'BusinessPayment',Amount:intAmount,PartyA:shortCode,PartyB:normalisedPhone,Remarks:purpose??'Creator Payout',QueueTimeOutURL:timeoutUrl,ResultURL:resultUrl,Occasion:purpose??'Testagram Payout'};
    ...
    const b2cData=await b2cRes.json(); console.log('[mpesa-b2c] Response:',JSON.stringify(b2cData));
    if(!b2cRes.ok||b2cData.ResponseCode!=='0'){const errMsg=b2cData.errorMessage??b2cData.ResponseDescription??`B2C failed (${b2cRes.status})`;throw new Error(`M-Pesa B2C: ${errMsg}`);}
    mpesaAccepted=true;

    const {error:txUpdateErr}=await supabaseAdmin.from('wallet_transactions').update({provider:'mpesa',provider_transaction_id:b2cData.ConversationID,reference:b2cData.OriginatorConversationID,amount_kes:intAmount,fx_rate:fxRate,payment_method:'mpesa',description:`M-Pesa withdrawal — KES ${intAmount.toLocaleString()} to ${normalisedPhone}`,metadata:{phone:normalisedPhone,purpose:purpose??'creator_payout'},updated_at:new Date().toISOString()}).eq('id',reservedTransactionId);
    if(txUpdateErr)throw new Error(`Payout accepted but local transaction update failed: ${txUpdateErr.message}`);

    const {error:mpesaInsertErr}=await supabaseAdmin.from('mpesa_transactions').insert({user_id:user.id,checkout_request_id:b2cData.ConversationID,merchant_request_id:b2cData.OriginatorConversationID,phone_number:normalisedPhone,amount:intAmount,currency:'KES',type:'b2c',purpose:purpose??'creator_payout',status:'pending',idempotency_key:idempotencyKey,wallet_transaction_id:reservedTransactionId,metadata:{fx_rate:fxRate}});
    ...
    await supabaseAdmin.from('platform_inbox').insert({user_id:user.id,subject:'Withdrawal Initiated 💸',body:`Your withdrawal of KES ${intAmount.toLocaleString()} ($${usdAmount.toFixed(2)}) to M-Pesa number ${normalisedPhone} has been initiated.`,type:'system',icon_emoji:'💸'});
    await supabaseAdmin.from('audit_logs').insert({actor_user_id:user.id,action:'mpesa_b2c_initiated',resource_type:'wallet_transaction',resource_id:reservedTransactionId,status:'success',metadata:{amount_kes:intAmount,amount_usd:usdAmount,fx_rate:fxRate}});
    ...
    if(reservedTransactionId&&!mpesaAccepted)await supabaseAdmin.rpc('release_wallet_reservation',{p_transaction_id:reservedTransactionId,p_reason:`M-Pesa request failed: ${message}`}).catch(()=>undefined);
