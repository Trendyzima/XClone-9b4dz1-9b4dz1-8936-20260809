...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!consumerKey || !consumerSecret || !shortCode || !passkey) {
      throw new Error('M-Pesa credentials must be configured in Edge Function secrets');
    }

    // STK is a wallet operation and must always be bound to the authenticated user.
    ...
    const { phone, amount, purpose, metadata, idempotency_key } = body;
    ...
    // Do not send a second STK request for a retried client request.
    const { data: existing } = await supabaseAdmin
      .from('mpesa_transactions')
      .select('id,checkout_request_id,merchant_request_id,status')
      .eq('user_id', user.id)
    ...
    const stkPayload = {
      ...
      TransactionDesc: purpose ?? 'XClone Wallet Top-Up',
    };
    ...
    if (insertErr) {
      // Safaricom accepted the request. Do not retry automatically here because
      // doing so could create a second customer prompt; reconciliation can use
      // the provider request IDs.
      console.error('[mpesa-stk] Ledger insert failed after provider acceptance:', insertErr.message);
      throw new Error('Payment prompt accepted but local ledger could not be updated; contact support');
    }
    ...
    await supabaseAdmin.from('audit_logs').insert({
      ...
      resource_id: stkData.CheckoutRequestID,
      status: 'success',
      metadata: { amount_kes: intAmount, phone: normalisedPhone, idempotency_key: idempotencyKey },
    });
